...
function view_info(info_element) {
    const info_container = document.getElementById('confirm_text_info');
    ...
    owner_name.textContent = "出店者名 " + info_element.店舗名;

    const owner_date = document.createElement('p');
    owner_date.className = 'COMMON_TEXT OWNER_TEXT';
    owner_date.textContent = "出店日時 " + info_element.日時;

    const confirm_button = document.createElement('button');
    confirm_button.textContent = "確定";
    confirm_button.className = 'COMMON_BUTTON COMMON_BUTTON_LARGE';
...
async function fetch_db(schedule_info) {
    ...
    const response = await fetch("http://127.0.0.1:8000/api/locate/place_owner_matching_accept/insert_datetime", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({
            store_info: schedule_info
        })
    });
