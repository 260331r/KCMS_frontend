...
document.addEventListener("DOMContentLoaded", () => {
    init_create_place_list();
    give_function_search_button();
});

// すべての日時のリストを作成する関数
async function init_create_place_list(){
...
async function create_place_list(date, list){
    const searched_elements = await db_search_elements(date);
    clear_error_text();
    create_place_frame_list(searched_elements, list);
    
    return 0;
}

// リストに表示する枠を作成する
function create_place_frame_list(shop_array, list){
...
async function db_all_elements(){
    const response = await fetch("http://127.0.0.1:8000/api/locate/shop_matching_request/get_store_info", {
        method: "POST",
        headers: {
...
async function db_search_elements(date){
    const response = await fetch("http://127.0.0.1:8000/api/locate/shop_matching_request/get_place_date", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
